...
export const fetchCountryWBIndicatorGDPPerCap = async (countryISO: `${string}${string}`) => {
  const response = await fetch(`https://api.worldbank.org/v2/country/${countryISO}/indicator/NY.GDP.PCAP.CD?format=json`);
  if (!response.ok) {
    throw new Error(`Error fetching data: ${response.statusText}`);
  }
  const data = await response.json();
