...
const HistoryModal = ({
  ...
  return (
    <div
      ref={modalRef}
      className={`${modalToggle ? "history-modal-open" : "modal-close"} w-1/2 h-auto max-h-[75%] rounded-lg bg-slate-400 border border-slate-600 z-10 text-slate-800 transition-transform duration-300`}
    >
        ...
        <div className="mb-2 overflow-y-auto">
          {historyData.length > 0 ? (
            historyData.map((gameState, idx) => (
              <div
                key={`${gameState.id}-${idx}`}
                className="game-record flex flex-col items-center justify-center my-4 text-left"
              >
                <span className="ml-3 text-left">
                  Date completed:{" "}
                  {convertDateStringToString(gameState.dateCompleted!)}
                </span>
