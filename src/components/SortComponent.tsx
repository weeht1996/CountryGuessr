...
const SortComponent = ({sortData, descending} : SortComponentProps) => {
    const [selectedType, setSelectedType] = useState<OptionValue>('dateCompleted');
    ...
    return (
        <div className="flex justify-center">
            <label htmlFor="critera" className="mx-2">Sort by:</label>
            <select name="criteria" id="criteria" className="font-semibold bg-slate-700 rounded-md text-slate-400" onChange={handleSelectChange}>
                <option value="dateCompleted" defaultChecked={true}>Date</option>
                <option value="totalPoints">Points</option>
            </select>
            <button className="ml-2 p-1 bg-slate-700 rounded-md" onClick={() => sortData(selectedType, descending)}>
            { (descending) ?
