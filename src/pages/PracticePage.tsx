...
import { fetchRestCountryData, fetchCountryWBIndicatorGDPPerCap } from "../services/restCountriesClient";
...
const PracticeQuestion = ({label, answer, interactiveMode, revealAnswer, updateQns} : PracticeQuestionProps ) => {

    const stdAnswer = useRef<string[]>(Array.isArray(answer) ? answer : [answer]);
    const lowerCaseAnswers = new Set(stdAnswer.current.map((ans) => ans.toLowerCase().replace(/[\s-]+/g, '')));
    ...
    return (
        <div className="flex gap-2 items-center">
            <label className="font-semibold">{label}:<span className="mr-2"></span>
            {   ( interactiveMode && !revealAnswer) ?
                <div className="inline">
                    <input className="rounded bg-slate-400 text-slate-800 w-40 h-4 indent-1 my-1" type="text" onChange={(e) => verifyInput(e)}/>
                </div>
                :
                <div className="inline">
                    {answer.toString()}
            ...
            {(revealAnswer && interactiveMode) && <img className="max-h-3 w-auto" src={`${process.env.PUBLIC_URL}/images/check.png `} alt="result-icon" />}
            </label>
        </div>
    )
};
...
const PracticeCard = ({cardState, interactiveMode, updateCardStatus} : PracticeCardProps) => {

    const [dataSetGDP, setDatasetGDP] = useState<GDPChartDataPoint[]>([{year:'N/A', value: 0}]);
    ...
    const [practiceQns, setPracticeQns] = useState([
        ...
        {qId: 3, label: 'Official Language(s)', answer: Object.values(country.languages), complete: false},
    ]);
            ...
            <div className="rounded-md relative items-center">
                <div className="title-body flex">
                    <div className="m-4 flex flex-col justify-center">
                        <div className="title-bar text-clamp-l font-semibold">{country.name}</div>
                        <img className="h-24 w-auto" src={country.flags} alt="country flag" />
                    </div>
                    <div className="flex flex-col items-start mx-4 text-clamp justify-center text-left">
                        <div className="flex justify-start mt-2">
                            ...
                            {dataSetGDP[0].year}, {(Math.round(dataSetGDP[0].value * 100) / 100).toLocaleString()} {(dataSetGDP && dataSetGDP.length > 0) && <TrendChart data={dataSetGDP}/>}
                        </div>
                        {
                ...
                >
                    ...
                    <div  className={`body flex flex-col gap-1 transition-all duration-500 ease-in-out overflow-hidden ${menuExpanded ? 'max-h-[60px]' : 'max-h-0'}`}>
...
const PracticePage = () => {
    ...
    const [filterVals, setFilterVals] = useState({region: 'All', subregion: 'All'});
    ...
    useEffect(() => {
        ...
        if (filterVals.subregion !== "All") {
            filtered = filtered.filter(item => item.country.subregion === filterVals.subregion);
        } else if (filterVals.region !== "All") {
            filtered = filtered.filter(item => item.country.region === filterVals.region);
        }
    ...
    const currentView = filteredCards.slice(indexWindow * cardsPerView, 
        ((indexWindow * cardsPerView + cardsPerView) >= filteredCards.length ) ? 
        filteredCards.length : indexWindow * cardsPerView + cardsPerView);
        ...
        <div className="practice-container flex justify-center">
            <div className="practice-body w-full lg:w-[1152px] flex flex-col gap-2 items-center justify-center">
                <div className="flex w-full my-4">
                    <button className={`w-1/2 rounded ${(interactiveMode) ? 'bg-slate-600' : 'bg-slate-300'} `} onClick={() => setInteractiveMode(false)}>Plain</button>
                    <button className={`w-1/2 rounded ${(!interactiveMode) ? 'bg-slate-600' : 'bg-slate-300'} `} onClick={() => setInteractiveMode(true)}>Interactive</button>
                </div>
                <div className="flex flex-col items-center w-full text-slate-300">
                    <div className="top-bar flex justify-between bg-slate-700 rounded-md w-full">
                        <div className="filters-container flex items-center mx-2 gap-2">
                            ...
                            <label htmlFor="region-select">
                                ...
                                <select className="text-slate-600 bg-slate-400 rounded-sm" id="region-select" onChange={(event) => {setFilterVals({region: event.target.value, subregion: 'All'})}}>
                                    {
                            ...
                            <label htmlFor="subregion-select">
                                ...
                                <select className="text-slate-600 bg-slate-400 rounded-sm" id="subregion-select" onChange={(event) => {setFilterVals((prev) => ({...prev, subregion: event.target.value}))}}>
                                    {   
                    ...
                    <div className="practice-body w-full">
                        <div key={resetCardKey} className="practice-card-container grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
                        {
                            currentView.map((cardState) => (
                                <PracticeCard key={cardState.id}
                                cardState={cardState} 
                    ...
                    <div className="bot-bar flex justify-center w-full gap-2 bg-slate-700 rounded-md">
                        <button onClick={() => handlePagination(false)}>
                            <svg className="-rotate-90" width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span className="m-1">Page: {indexWindow + 1}/{totalPages}</span>
