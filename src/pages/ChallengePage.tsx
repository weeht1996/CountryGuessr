...
const ChallengePage = () => {
  ...
  const userSubmitAnswer = () => {
    ...
    const updatedCardStates: CardState[] = gameState.cardStates.map((item) =>
      item.country.name === userInputValue ? { ...item, guessed: true } : item,
    );
    ...
    setGameState((prev) => ({
      ...
      totalPoints:
        prev.totalPoints +
        (gameState.cardStates.find(
          (item) => item.country.name === userInputValue,
        )?.points ?? 0),
    ...
    if (countryOptions.length !== 0 && isCorrect) {
      const updatedCountryOptions = countryOptions.map((item) =>
        item.value === userInputValue ? { ...item, isDisabled: true } : item,
      );
  ...
  const userGaveUp = (countryName: string) => {
    setGameState((prev) => ({
      ...
      cardStates: prev.cardStates.map((item) =>
        item.country.name === countryName ? { ...item, gaveUp: true } : item,
      ),
    }));
  };
  ...
  const SubtractPoints = (
    ...
    hintCost: number,
    hintName: string,
    revealAllHints: boolean = false,
  ) => {
    const updatedPointsFilteredData = revealAllHints
      ? gameState.cardStates.map((item) =>
  ...
  const filterData = (unfilteredData: DataCountry[]) => {
    ...
    const shuffledData = unfilteredData
      .slice()
  ...
  useEffect(() => {
    ...
    setGameState((prev) => ({
      ...prev,
      cardStates: prev.cardStates.map((item) =>
        item.guessed ? item : { ...item, gaveUp: true },
      ),
  ...
  useEffect(() => {
    if (
      gameState.dateCompleted &&
      (allGames.length === 0 ||
        gameState.id !== allGames[allGames.length - 1].id)
    ) {
  ...
  return (
    <div className="w-full flex flex-col items-center text-slate-300 text-clamp min-h-[75vh] mb-10 relative">
      <ChallengeHeaderBar
        ...
        allRecords={allGames}
        guideModalToggle={guideModalToggle}
      ...
      <div className="user-section flex gap-1 sm:gap-8 justify-between h-10 w-full max-w-[1152px] mb-2">
        ...
        <input
          type="button"
          className={`w-1/6 bg-slate-800 rounded-md ${!isGameOver && "hover:cursor-pointer"}`}
          value="Submit"
          onClick={userSubmitAnswer}
          disabled={isGameOver}
        />
      </div>
      <AttemptLog
        ...
        attempts={gameState.prevAttempts}
        attemptLimit={maxAttemptLimit}
      />
      {filterData.length !== 0 && (
        <CountryCardList
          ...
          handleGiveUp={userGaveUp}
        ></CountryCardList>
      )}
      <PostGameModal
        totalPoints={gameState.totalPoints}
        correctAnswers={gameState.totalCorrect}
        toggled={isPostGameModalOpen}
